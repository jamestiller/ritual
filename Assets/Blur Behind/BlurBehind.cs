...
public class BlurBehind : MonoBehaviour
{
    ...
    void Downsample(RenderTexture source, RenderTexture dest, int width, int height)
    {
        int downsampleLeft = 0;
        if (source.width > source.height)
        {
            int testLength = source.width;
            while (testLength > width)
            {
                downsampleLeft++;
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        int outputWidth;
        int outputHeight;
        OutputSetting(croppedSource, out outputWidth, out outputHeight);

        CheckOutput(outputWidth, outputHeight, croppedSource.format);

        Downsample(croppedSource, storedTexture, outputWidth, outputHeight);

        // Release cropped if was used
    ...
    void OutputSetting(RenderTexture source, out int width, out int height)
    {
        if (mode == Mode.Absolute)
        {
            width = Mathf.RoundToInt((float)source.width / downsample);
            height = Mathf.RoundToInt((float)source.height / downsample);
        }
        else
        {
            if (source.width > source.height)
            {
                ...
                {
                    width = Mathf.RoundToInt(downsample);
                    height = Mathf.RoundToInt(downsample / source.width * source.height);
                }
                else
                {
                    width = source.width;
            ...
            else
            {
                ...
                {
                    width = Mathf.RoundToInt(downsample / source.height * source.width);
                    height = Mathf.RoundToInt(downsample);
                }
                else
                {
                    width = source.width;
